...
-- ──────────────────────────────────────────────────────────────────────────────
-- PCS Global Observatory Network – visitor sessions
--
-- Privacy:
-- - No full IP address is stored.
-- - Geographic values are approximate and come from Cloudflare request.cf.
-- - session_id is a random browser UUID stored in localStorage.
-- ──────────────────────────────────────────────────────────────────────────────
...
-- Each full page visit creates one event.
-- Ping requests must not create rows in this table.
...
CREATE INDEX IF NOT EXISTS idx_visitor_sessions_country
  ON visitor_sessions (country_code);
...
CREATE INDEX IF NOT EXISTS idx_visitor_events_location
  ON visitor_events (country_code, city);
