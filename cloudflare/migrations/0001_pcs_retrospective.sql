-- Additive production migration for PCS retrospective analysis.
-- Intentionally does not touch legacy PCS or visitor tables.

CREATE TABLE IF NOT EXISTS pcs_events (
  id TEXT PRIMARY KEY, cluster_id TEXT, title TEXT NOT NULL, category TEXT NOT NULL,
  region TEXT NOT NULL, event_type TEXT NOT NULL, event_summary TEXT, why_it_matters TEXT,
  research_relevance TEXT, published_at TEXT, observed_event_time TEXT, source_url TEXT,
  source_name TEXT, source_type TEXT, image_url TEXT, confidence REAL,
...
CREATE TABLE IF NOT EXISTS pcs_event_sources (
  id INTEGER PRIMARY KEY AUTOINCREMENT, event_id TEXT NOT NULL REFERENCES pcs_events(id),
  source_url TEXT NOT NULL, source_name TEXT, source_type TEXT, published_at TEXT,
...
CREATE TABLE IF NOT EXISTS pcs_retrospective_analyses (
  analysis_id TEXT PRIMARY KEY, event_id TEXT NOT NULL REFERENCES pcs_events(id),
  result_time TEXT, analysis_start_time TEXT, precursor_window_start TEXT,
  precursor_window_end TEXT, earliest_detectable_time TEXT, estimated_lead_time_hours REAL,
  region TEXT, affected_systems TEXT NOT NULL DEFAULT '[]', precursor_signals TEXT NOT NULL DEFAULT '[]',
  causal_chain TEXT NOT NULL DEFAULT '[]', amplification_factors TEXT NOT NULL DEFAULT '[]',
  exposure_factors TEXT NOT NULL DEFAULT '[]', data_sources_used TEXT NOT NULL DEFAULT '[]',
  missing_data TEXT NOT NULL DEFAULT '[]', pcs_observability TEXT NOT NULL DEFAULT '{}',
  proposed_warning_rules TEXT NOT NULL DEFAULT '[]', proposed_interventions TEXT NOT NULL DEFAULT '[]',
  validation_status TEXT NOT NULL DEFAULT 'unvalidated', validation_notes TEXT, analyst_confidence REAL,
  created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ','now')),
  updated_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ','now'))
);
...
CREATE TABLE IF NOT EXISTS pcs_event_timeline (
  id INTEGER PRIMARY KEY AUTOINCREMENT, event_id TEXT NOT NULL REFERENCES pcs_events(id),
  milestone_type TEXT NOT NULL, occurred_at TEXT,
  value_status TEXT NOT NULL CHECK(value_status IN ('observed','inferred','estimated','unavailable','delayed')),
  description TEXT, source_url TEXT, confidence REAL,
  created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ','now'))
);

CREATE TABLE IF NOT EXISTS pcs_warning_rules (
  id TEXT PRIMARY KEY, event_type TEXT NOT NULL, version TEXT NOT NULL, conditions TEXT NOT NULL,
  data_sources TEXT NOT NULL, test_results TEXT NOT NULL DEFAULT '{}', false_positive_rate REAL,
  false_negative_rate REAL, status TEXT NOT NULL DEFAULT 'candidate',
  created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ','now')),
  updated_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ','now')),
...
CREATE TABLE IF NOT EXISTS pcs_evidence_ledger (
  analysis_id TEXT PRIMARY KEY, event_id TEXT REFERENCES pcs_events(id), issued_at TEXT NOT NULL,
  region TEXT NOT NULL, event_type TEXT NOT NULL, expected_event_window TEXT,
  input_data_snapshot TEXT NOT NULL DEFAULT '[]', precursor_signals TEXT NOT NULL DEFAULT '[]',
  causal_chain TEXT NOT NULL DEFAULT '[]', warning_rule_version TEXT, confidence REAL,
  proposed_actions TEXT NOT NULL DEFAULT '[]', actual_event TEXT, official_confirmation_time TEXT,
  news_publication_time TEXT, lead_time_hours REAL,
  result TEXT NOT NULL CHECK(result IN ('confirmed','partially_confirmed','false_alarm','missed','unresolved','insufficient_data')),
  false_positive INTEGER NOT NULL DEFAULT 0, false_negative INTEGER NOT NULL DEFAULT 0,
  partial_hit INTEGER NOT NULL DEFAULT 0, data_missing INTEGER NOT NULL DEFAULT 0,
  retrospective_score REAL, lessons_learned TEXT, model_revision TEXT, reviewed_at TEXT,
  created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ','now'))
);

CREATE TABLE IF NOT EXISTS pcs_data_snapshots (
  id TEXT PRIMARY KEY, event_id TEXT REFERENCES pcs_events(id), provider TEXT NOT NULL,
  dataset TEXT NOT NULL, endpoint TEXT NOT NULL, timestamp TEXT, spatial_resolution TEXT,
  temporal_resolution TEXT, latency REAL, license TEXT, quality_flag TEXT, uncertainty REAL,
  retrieval_status TEXT NOT NULL, payload TEXT,
  retrieved_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ','now'))
);

CREATE TABLE IF NOT EXISTS pcs_mass_gatherings (
  id TEXT PRIMARY KEY, case_id TEXT, event_name TEXT NOT NULL, event_category TEXT NOT NULL,
  venue TEXT, city TEXT, region TEXT, country TEXT, latitude REAL, longitude REAL,
  start_time TEXT, end_time TEXT, estimated_attendance REAL, venue_capacity REAL,
  outdoor_exposure TEXT, queue_duration_estimate REAL, transport_load REAL,
  international_arrival_estimate REAL, domestic_arrival_estimate REAL, hotel_occupancy REAL,
  airport_load REAL, rail_load REAL, road_congestion REAL, medical_capacity REAL,
  shade_availability REAL, water_availability REAL, hazard_index REAL, crowd_density REAL,
  vulnerability REAL, exposure_duration REAL, risk_index REAL, source TEXT, confidence REAL,
  data_status TEXT NOT NULL DEFAULT 'unavailable', source_limitations TEXT,
  created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ','now')),
  updated_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%SZ','now'))
);

CREATE TABLE IF NOT EXISTS pcs_human_mobility (
  id TEXT PRIMARY KEY, case_id TEXT, city TEXT, region TEXT, country TEXT, observed_at TEXT,
  metric TEXT NOT NULL, observed_total REAL, estimated_related REAL, confidence_interval_low REAL,
  confidence_interval_high REAL, estimation_method TEXT, data_latency REAL, source TEXT,
  source_url TEXT,
...
CREATE TABLE IF NOT EXISTS pcs_provider_status (
  adapter_id TEXT PRIMARY KEY, provider TEXT NOT NULL, dataset TEXT NOT NULL, endpoint TEXT NOT NULL,
  timestamp TEXT, latency REAL, quality_flag TEXT, uncertainty REAL, license TEXT,
  validation_status TEXT, availability TEXT, retrieval_status TEXT, checked_at TEXT NOT NULL
);

CREATE TABLE IF NOT EXISTS pcs_scheduled_runs (
  id TEXT PRIMARY KEY, cron TEXT NOT NULL, started_at TEXT NOT NULL, completed_at TEXT,
  status TEXT NOT NULL, details TEXT
);

-- Metadata-only case shells. No observation, estimate, confidence, or lead-time value is inserted.
...
INSERT OR IGNORE INTO pcs_retrospective_analyses
  (analysis_id, event_id, region, affected_systems, precursor_signals, causal_chain, amplification_factors,
   exposure_factors, data_sources_used, missing_data, pcs_observability, proposed_warning_rules,
   proposed_interventions, validation_status, validation_notes)
VALUES
