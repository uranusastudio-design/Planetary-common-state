...
| 6 | NASA FIRMS - Wildfire / Active Fire | Biosphere | Wildfire | Active fire detections | NASA FIRMS | VIIRS active fire; MODIS active fire | Near-real-time to daily | CSV, GeoJSON, shapefile, or API response | Connector implemented v1.0; data access pending unless real source was successfully loaded | `PCS_CONNECTORS/nasa_firms_wildfire/` | Event data require careful spatial and temporal aggregation. |
| 7 | NASA IMERG - Precipitation | Hydrology | Precipitation | Precipitation rate or accumulation | NASA GPM IMERG | GPM; ERA5 precipitation | Half-hourly, daily, monthly, or annual aggregate | HDF5, NetCDF, GeoTIFF, CSV, JSON, or documented subset | Connector implemented v1.0; data access pending unless real source was successfully loaded | `PCS_CONNECTORS/nasa_gpm_imerg/` | Connector must preserve aggregation period. |
...
| 9 | Argo - Ocean Temperature / Salinity | Ocean | Ocean Interior | Ocean temperature and salinity profiles | International Argo Programme | Copernicus Marine Service | Near-real-time and delayed-mode updates | NetCDF, CSV, JSON, or documented profile summary | Connector implemented v1.0; data access pending unless real source was successfully loaded | `PCS_CONNECTORS/argo_ocean/` | Profile data require depth-aware handling before PCS aggregation. |
