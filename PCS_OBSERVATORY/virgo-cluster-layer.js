(function exposeVirgoCluster(global){
  ...
  class VirgoClusterLayer{
    ...
    updateLod(force=false){if(!this.clusterPoint)return this.lod;const center=scene(this.cluster,this.mode),distance=Cesium.Cartesian3.distance(this.viewer.camera.positionWC,center),unit=Coordinates.sceneRadiusMpc(5,this.mode,"virgo"),ratio=distance/unit,next=force?(ratio>2.2?"far":ratio>.55?"mid":"near"):this.lod==="far"?(ratio<2.05?(ratio<.48?"near":"mid"):"far"):this.lod==="near"?(ratio>.62?(ratio>2.35?"far":"mid"):"near"):(ratio>2.35?"far":ratio<.48?"near":"mid");if(!force&&next===this.lod)return this.lod;this.setLod(next);return next;}
