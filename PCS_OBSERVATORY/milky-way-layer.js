(function exposeMilkyWayLayer(global) {
  ...
  class MilkyWayLayer {
    ...
    constructor(viewer){this.viewer=viewer;this.points=null;this.lines=null;this.labels=null;this.records=[];this.pointRecords=[];this.mode="exhibition";this.reconstruction=true;this.visible=false;}
    load(records,mode="exhibition",options={}){this.unload();this.mode=mode;this.records=[MilkyWayLayer.SUN,MilkyWayLayer.SGR_A,...records];this.points=this.viewer.scene.primitives.add(new Cesium.PointPrimitiveCollection());this.lines=this.viewer.scene.primitives.add(new Cesium.PolylineCollection());this.labels=this.viewer.scene.primitives.add(new Cesium.LabelCollection());this.addStructure();const cap=options.maxObjects||records.length;for(const record of this.records.slice(0,cap+2)){const landmark=record.id.startsWith("milky-way:"),position=scene(record.galactocentricCartesianKpc,mode),point=this.points.add({position,pixelSize:landmark?9:3.2,color:Cesium.Color.fromCssColorString(landmark?"#fff4cf":ARM_COLORS[record.spiralArmCode]||"#8eb8cb").withAlpha(landmark?1:.82),outlineColor:Cesium.Color.BLACK,outlineWidth:landmark?2:0,id:{phase3Object:record}});this.pointRecords.push([point,record,landmark]);if(landmark)this.labels.add({position,text:record.canonicalName,font:"13px system-ui",fillColor:Cesium.Color.WHITE,outlineColor:Cesium.Color.BLACK,outlineWidth:3,style:Cesium.LabelStyle.FILL_AND_OUTLINE,pixelOffset:new Cesium.Cartesian2(9,-9)});}
