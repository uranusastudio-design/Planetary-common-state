...
test("Milky Way scale transition keeps the adopted Galactocentric frame",()=>{assert.match(source,/galactocentricRecord/);assert.match(source,/heliocentricGalacticToGalactocentric/);assert.match(source,/galactocentricCartesianKpc/);assert.match(source,/PCS Galactic Center anchor/);});
test("Local Group transition does not invent a rigid circular boundary",()=>{assert.match(source,/No rigid Local Group boundary is rendered/);assert.doesNotMatch(source,/const radius=1500|samples=128/);});
