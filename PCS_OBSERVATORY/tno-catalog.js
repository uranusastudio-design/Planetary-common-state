(function exposeTnoCatalog(global){
  ...
  class TnoLayer{
    ...
    debug(){return Object.freeze({datasetId:dataset.datasetId,knownCatalogRecords:this.records.length,pointCount:this.points.length,visibleCount:this.points.filter(point=>point.show).length,lod:this.lod,deterministic:true,epochUpdatePending:Boolean(this.pendingEpoch||this.updateTargetEpoch),epochUpdateCursor:this.updateCursor,completedEpochUpdates:this.completedEpochUpdates,selection:dataset.selection,representativePopulation:dataset.representativePopulation});}
  }
