(function initializeReleaseCenter() {
  ...
  const statusText = (status) => copy()[status === "in-progress" || status === "preview" ? "inProgress" : status] || status;
  const itemStatusText = (item) => localize(item.statusLabel) || statusText(item.status);
  ...
  function renderRoadmap() {
    const c = copy();
    const symbols = {completed:"✅","in-progress":"▶",planned:"○",waiting:"⏸",blocked:"!",deferred:"◇"};
    return `<h3>${c.currentProgress}</h3><p class="pcs-release-notice">${escapeHtml(registry.currentVersion)} · ${escapeHtml(registry.roadmap.find(item => item.status === "in-progress")?.title || "")} ${escapeHtml(c.next)}. No numeric percentage is asserted.</p><ol class="pcs-roadmap">${registry.roadmap.map(item => `<li data-status="${escapeHtml(item.status)}"><span class="pcs-roadmap-symbol">${escapeHtml(item.symbol || symbols[item.status])}</span><div><strong>${escapeHtml(item.title)}</strong><span>${escapeHtml(item.detail)}</span>${item.items?.length ? `<ul class="pcs-roadmap-items">${item.items.map(value => `<li>${escapeHtml(value)}</li>`).join("")}</ul>` : ""}</div><span class="pcs-release-status" data-status="${escapeHtml(item.status)}">${escapeHtml(itemStatusText(item))}</span></li>`).join("")}</ol>`;
  }
