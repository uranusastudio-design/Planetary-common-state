...
export function buildCometLookupQuery(designation){const url=new URL(buildLookupQuery(designation));url.searchParams.set("alt-des","true");url.searchParams.set("cd-epoch","true");url.searchParams.set("cd-tp","true");return url.toString();}
...
export function normalizeLookupResponse(payload,classification="dwarf planet"){
  ...
  const name=String(payload.object.shortname||payload.object.fullname||payload.object.des).replace(/^\s*\d+[A-Z]?\/?\s*/,"").trim();
  return Object.freeze({spkid:String(payload.object.spkid),fullName:payload.object.fullname,name,designation:payload.object.des,classification,orbitClass:payload.object.orbit_class?.code||null,orbitClassName:payload.object.orbit_class?.name||null,orbitId:payload.object.orbit_id||null,absoluteMagnitude:finiteOrNull(physical.H?.value),diameterKm:finiteOrNull(physical.diameter?.value),elements:Object.freeze({e:value("e"),a:value("a"),q:value("q"),i:value("i"),om:value("om"),w:value("w"),ma:value("ma"),tp:value("tp"),epochJdTdb:finiteOrNull(payload.orbit.epoch),periodDays:value("per"),aphelionAu:value("ad"),meanMotionDegPerDay:value("n")}),uncertainty:Object.freeze({e:sigma("e"),aAu:sigma("a"),qAu:sigma("q"),iDeg:sigma("i"),omDeg:sigma("om"),wDeg:sigma("w"),maDeg:sigma("ma"),tpDays:sigma("tp"),periodDays:sigma("per")}),conditionCode:payload.orbit.condition_code??null,dataArcDays:finiteOrNull(payload.orbit.data_arc),observationsUsed:finiteOrNull(payload.orbit.n_obs_used),source:"NASA/JPL SBDB",referenceFrame:`${payload.orbit.equinox||"J2000"} ecliptic heliocentric osculating elements`,solutionEpochTimeScale:"TDB"});
}

export function normalizeCometLookupResponse(payload){
  const base=normalizeLookupResponse(payload,"comet"),element=Object.fromEntries((payload.orbit?.elements||[]).map(item=>[item.name,item])),aliases=(payload.object?.des_alt||[]).flatMap(item=>Object.values(item)).filter(Boolean);
  return Object.freeze({...base,prefix:payload.object?.prefix||null,kind:payload.object?.kind||null,aliases:Object.freeze(aliases),orbitSolutionDate:payload.orbit?.soln_date||null,firstObservation:payload.orbit?.first_obs||null,lastObservation:payload.orbit?.last_obs||null,perihelionCalendarTdb:element.tp_cd?.value||null,modelParameters:Object.freeze((payload.orbit?.model_pars||[]).map(item=>Object.freeze({name:item.name,kind:item.kind,value:finiteOrNull(item.value),sigma:finiteOrNull(item.sigma),units:item.units||null,title:item.title||item.name}))) });
}
