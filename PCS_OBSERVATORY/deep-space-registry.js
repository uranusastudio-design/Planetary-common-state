(function exposeDeepSpaceRegistry(global) {
  ...
  satellites.forEach(([id,name,parentBodyId,naifId,radiusKm,meanOrbitalRadiusKm,orbitalPeriodDays,rotationPeriodDays,inclinationDeg,eccentricity,color]) => {
    const orbitalDataSource="NASA/JPL Horizons cached parent-relative VECTORS";
    const satelliteFrame=`ICRF; Earth mean ecliptic at J2000.0 (IAU76/80); ${parentBodyId}-centered origin`;
    registry[id] = Object.freeze({ id,name,type:"natural-satellite",parentBodyId,naifId,radiusKm,meanOrbitalRadiusKm,orbitalPeriodDays,rotationPeriodDays,inclinationDeg,eccentricity,coordinateFrame:satelliteFrame,ephemerisSource:JPL_HORIZONS,orbitalDataSource,textureSource:`Existing PCS mission imagery registry (${id})`,dataStatus:"ephemeris-derived when inside promoted cache coverage",uncertainty:"Horizons VECTORS supplies no covariance; PCS interpolation is independently checked against withheld epochs.",visualizationScale:"parent-system compressed; physical radius retained separately",visualizationStatus:"mission imagery / ephemeris position / visually exaggerated system spacing",color,orbit:Object.freeze({objectId:id,parentBodyId,dataSource:orbitalDataSource,preferredDataSource:JPL_HORIZONS,epoch:"Requested Display Epoch (UTC converted to TDB)",coordinateFrame:satelliteFrame,validTimeRange:Object.freeze({start:"2025-01-01T00:00:00.000Z",end:"2028-01-01T00:00:00.000Z"}),sampleIntervalDays:Math.abs(orbitalPeriodDays)/120,precisionStatus:"Cached ephemeris · cubic Hermite",renderStatus:"available within validated cache coverage when the parent planetary system is focused",fallbackStatus:"Unavailable outside promoted coverage; representative mean orbit disabled",periapsisKm:meanOrbitalRadiusKm*(1-eccentricity),apoapsisKm:meanOrbitalRadiusKm*(1+eccentricity),inclinationDeg}) });
  });
